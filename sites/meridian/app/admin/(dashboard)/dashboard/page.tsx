...
import { getDashboardStats, listLeads, listProjects } from "@/lib/data";
...
export default function AdminDashboardPage() {
  ...
  const allProjects = listProjects({ publishedOnly: false });
  ...
  return (
    <div className="space-y-8">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-[11px] uppercase tracking-wide text-ink/40 border-b border-ink/10">
                <th className="pb-2 pr-4">Name</th><th className="pb-2 pr-4">Project</th><th className="pb-2 pr-4">Type</th><th className="pb-2 pr-4">Status</th><th className="pb-2">Received</th>
              </tr>
            </thead>
            <tbody>
              {recentLeads.map((l) => (
                <tr key={l.id} className="border-b border-ink/5 last:border-0">
                  ...
                  <td className="py-3 pr-4 text-ink/60">{l.project_title || <span className="text-ink/30">General enquiry</span>}</td>
                  <td className="py-3 pr-4 text-ink/60 capitalize">{l.type.replace("_", " ")}</td>
                  <td className="py-3 pr-4">
                    <span className={`text-[10px] font-semibold uppercase tracking-wide px-2 py-0.5 ${LEAD_STATUS_COLORS[l.status]}`}>{LEAD_STATUS_LABELS[l.status]}</span>
                  </td>
                  <td className="py-3 text-ink/40">{timeAgo(l.created_at)}</td>
                </tr>
              ))}
              {recentLeads.length === 0 && (
                <tr><td colSpan={5} className="py-8 text-center text-ink/40">No leads yet — submissions from your public site will show up here automatically.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div className="flex items-center gap-2 text-sm text-ink/40">
        <Building2 size={16} />
        {publishedProjects} of {totalProjects} projects published
      </div>
    </div>
  );
}
