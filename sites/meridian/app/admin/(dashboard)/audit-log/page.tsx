        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-[11px] uppercase tracking-wide text-white/40 border-b border-white/10">
              <th className="p-4">Actor</th><th className="p-4">Role</th><th className="p-4">Action</th><th className="p-4">Entity</th><th className="p-4">When</th>
            </tr>
          </thead>
          <tbody>
            {entries.map((e) => (
              <tr key={e.id} className="border-b border-white/5 last:border-0">
                ...
                <td className="p-4 text-white/40 text-xs uppercase">Admin</td>
                <td className="p-4 text-white/60 capitalize">{e.action}</td>
                <td className="p-4 text-white/60"><span className="capitalize">{e.entity}</span></td>
                <td className="p-4 text-white/30">{formatDateTime(e.created_at)}</td>
              </tr>
            ))}
            {entries.length === 0 && (
              <tr><td colSpan={5} className="p-8 text-center text-white/40">No actions recorded yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>
